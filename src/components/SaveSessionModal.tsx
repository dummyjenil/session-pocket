...
import type { ActiveTabInfo, ColorTag } from "~types"

interface SaveSessionModalProps {
  isOpen: boolean
  tabInfo: ActiveTabInfo | null
  onClose: () => void
  onSave: (name: string, tag: ColorTag, notes?: string) => Promise<void>
  isSaving: boolean
}
...
export const SaveSessionModal: React.FC<SaveSessionModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await onSave(name.trim(), selectedTag, notes.trim() || undefined)
    onClose()
  }
      ...
      >
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="text-xs font-semibold">Save New Session Pocket</h3>
              <p className="text-[10px] text-zinc-400 truncate max-w-[200px]">
                {tabInfo?.domain || "Current website"}
              </p>
            </div>
          </div>
          <button
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          <div>
            ...
            <input
              ...
              placeholder="e.g. Work, Personal, Creator, Alt #1"
          ...
          <div>
            ...
            <input
              ...
              placeholder="e.g. 2FA enabled, client testing"
              className="w-full px-3 py-1.5 bg-zinc-950 border border-zinc-800 rounded-lg text-xs text-zinc-100 placeholder-zinc-500 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500"
            />
          </div>

          <div className="p-2 rounded-lg bg-zinc-950/70 border border-zinc-800/80 text-[10px] text-zinc-400 space-y-0.5">
            <p className="text-zinc-300 font-medium">Session State Captured:</p>
            <p>✓ All domain & authentication cookies</p>
            <p>✓ LocalStorage & SessionStorage state</p>
            <p>✓ IndexedDB auth tokens & data</p>
          </div>
